...
This document describes how `efcore-paradedb` releases are cut and what
compatibility guarantees the project provides.

## Goals

- Keep the Entity Framework Core integration stable and predictable.
...
`efcore-paradedb` follows SemVer:

- MAJOR: Breaking API or behavior changes
- MINOR: Backward-compatible features
- PATCH: Backward-compatible fixes and docs updates

The NuGet package version is supplied as the `version` input to the release
workflow and embedded into the package at pack time. The workflow validates
that the requested version is well-formed and that the corresponding tag and
GitHub release do not already exist.

## Compatibility Principles

1. Major-version alignment after ParadeDB 1.0: we plan to align majors where
   practical (for example, ParadeDB 1.x with `efcore-paradedb` 1.x).
...
3. Confirm `README.md` and examples reflect shipped behavior.

## Release Workflow

Use GitHub Actions workflow `Release` (`.github/workflows/release.yml`) with:

- `version`: release version (semver, e.g. `0.4.0`)
...
3. Pack `ParadeDB.EntityFrameworkCore.<version>.nupkg`.
4. Publish the package to NuGet.
5. Create the GitHub release, which tags the target commit and attaches the
   built `.nupkg`.
